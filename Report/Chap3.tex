...
Chapter 3 focuses on presenting the simulation resuls of the designed OFDM system and also do some analyses of the performance and characteristics of the system.
...
The program reads data from an input image file and obtains all the pixels value (from all 3 channels). This matrix is then rearranged into a 1D array. This array is gonna have a size of $width*height*channel = 256*256*3 = 196608$ (bytes).
...
For further analysis, we the varying SNRDB from 0 to 40 db and then calculating the error bits using CRC decode. Dividing the total number of errors by total number of
demodulated symbols, the bit-error-rate (BER) is found. Figure \ref{ber} show the plot of BER with respecte to SNR.
